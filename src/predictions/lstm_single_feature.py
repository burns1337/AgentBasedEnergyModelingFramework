...
def plot_predictions(train, valid, predictions, rmse, mae, mape):
    ...
    plt.title(f'Energy Price Prediction Austria with a LSTM Model. First Draft  (RMSE: {rmse:.2f}, MAE: {mae:.2f}, MAPE: {mape:.2f}%)', fontsize=16)
    plt.xlabel('Date', fontsize=15)
    plt.ylabel('Energy Prices [EUR/MWh]', fontsize=15)
    plt.plot(train['Preis MC Auktion [EUR/MWh]'])
    plt.plot(valid[['Preis MC Auktion [EUR/MWh]', 'Predictions']])
...
def main():
    ...
    energy_prices_df.index = pd.to_datetime(energy_prices_df.index, format='%d.%m.%Y %H:%M:%S')

    # Preprocess the data
    # feature_col = energy_prices_df.columns.tolist()
    feature_col = 'Preis MC Auktion [EUR/MWh]'
    data, scaler = preprocess_data(energy_prices_df, feature_col)
    ...
    for i in range(10):
        ...
        train = energy_prices_df[:train_size]
        valid = energy_prices_df[train_size:train_size + len(predictions)]
        valid['Predictions'] = predictions.flatten()
        # print(valid)

        # Calculate error metrics
        mae = mean_absolute_error(valid['Preis MC Auktion [EUR/MWh]'], valid['Predictions'])
        mse = mean_squared_error(valid['Preis MC Auktion [EUR/MWh]'], valid['Predictions'])
        rmse = np.sqrt(mse)
        actual_values = valid['Preis MC Auktion [EUR/MWh]'].values
